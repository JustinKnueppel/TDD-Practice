...
const getDollarString = (dollars: number): string => {
  ...
  while (dollars > 0) {
    ...
    const thousandsString =
      modifierCount > 0
        ? `${hundredsString} ${modifiers[modifierCount]}`
        : hundredsString;
    if (hundredsString) dollarStringParts.splice(0, 0, thousandsString);
...
const parseHundreds = (hundreds: number): string => {
  ...
  const hundredsString =
    numberOfHundreds > 0 ? `${parseUnder20(numberOfHundreds)} hundred` : "";
